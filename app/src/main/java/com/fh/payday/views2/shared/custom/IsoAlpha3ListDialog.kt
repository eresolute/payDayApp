...
private class ItemAdapter(
    private val list: List<IsoAlpha3>,
    private val countrySelectListener: OnCountrySelectListener,
    private val dialog: Dialog,
    private val activity: BaseActivity
//    private val block: (Int) -> Unit
) : RecyclerView.Adapter<ViewHolder>() {

    private var items = ArrayList<IsoAlpha3>()
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.rootView.setOnClickListener {
            countrySelectListener.onCountrySelect(items[position])
            activity.window.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN)
            dialog.dismiss()
        }
    }

    fun filter(query: String): List<IsoAlpha3> {
        items.clear()

        list.filter {
            it.code.startsWith(query, true) || it.country.startsWith(query, true)
...
private class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
    ...
    fun bindTo(isoAlpha3: IsoAlpha3) {
        tvCountry.text = isoAlpha3.country

        if (isoAlpha3.code.isEmpty()) tvAlpha3.visibility = View.GONE
            val alpha3 = "(${isoAlpha3.code})"
