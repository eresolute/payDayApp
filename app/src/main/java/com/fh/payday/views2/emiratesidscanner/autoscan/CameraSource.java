...
import com.fh.payday.utilities.CameraUtils;
...
public class CameraSource {
  ...
  private Camera createCamera() throws IOException {
    ...
    //Camera.Size size = getOptimalPreviewSize(sortedSizes, aspectRatio.getX(), aspectRatio.getY());
    int width = activity.getResources().getDisplayMetrics().widthPixels;
    int height = activity.getResources().getDisplayMetrics().heightPixels;

    Camera.Size size = CameraUtils.INSTANCE.getOptimalPreviewSize(
    ...
    if (size != null) {
      sizePair = CameraSource.selectSizePair(
          camera,
          size.width,
          size.height);
    }
  ...
  void takePicture(CaptureCallback callback) {
      ...
      camera.autoFocus((success, camera) -> takePictureInternal(callback));*/
  }

  private void takePictureInternal(CaptureCallback callback) {
  ...
  public boolean setFlashMode(int flash) throws IllegalStateException {
    if (camera != null) {
      Camera.Parameters parameters = camera.getParameters();
      ...
      throw new IllegalStateException("Unexpected flash type " + flash);
    } else {
      return false;
    }
  }

  public int getFlashMode() throws IllegalStateException {
    if (camera != null) {
      Camera.Parameters parameters = camera.getParameters();
      String mode = parameters.getFlashMode();

      int currentMode = -1;
      for(int i = 0; i < FLASH_MODES.size(); ++i) {
        if (FLASH_MODES.valueAt(i).equalsIgnoreCase(mode)) {
          currentMode = FLASH_MODES.keyAt(i);
        }
      }

      if (currentMode == -1) {
        return Constants.FLASH_OFF;
      }
