package com.fh.payday.biometricauth
...
import com.fh.payday.biometricauth.v28.AuthenticationCallback

@RequiresApi(Build.VERSION_CODES.M)
class BiometricPromptCompat  constructor(
    ...
    private  var biometricPrompt: com.fh.payday.biometricauth.v23.BiometricPrompt ?= null
    fun authenticate(callback: Callback) {
        when {
            !hasSDKSupport() -> { return callback.onSDKNotSupported() }
            !hasHardwareSupport(context) -> { return callback.onHardwareNotDetected() }
            !isPermissionsGranted(context) -> { return callback.onPermissionsNotGranted() }
            !isFingerprintAvailable(context) -> { return callback.onBiometricAuthNotAvailable() }
        }
    ...
    private fun showBiometricPromptV23(listener: OnAuthenticateListener) {
        biometricPrompt = com.fh.payday.biometricauth.v23.BiometricPrompt.Builder(context)
                .setTitle(title)
                .setSubtitle(subtitle)
                .setDescription(description)
                .setButtonNegative(btnText)
                .build()
               biometricPrompt?.authenticate(listener)
    }
